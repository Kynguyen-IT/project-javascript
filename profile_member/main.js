...
function displayData(){
  ...
  .then((user) => {
    if(user['image'] !== null && user['image'] !== undefined){
      ...
      image_member.style.backgroundImage = 'url(https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcSteItzPyeDKBxyWiOA8xrPZXIlxOYv1b1VVg&usqp=CAU)' 
    }
  
    if(user['name'] != undefined || user['phone'] != undefined || user['address'] != undefined ){
      // show data in page
...
function update(){
  ...
  if(nameValue !== '' && emailValue !== '' && phoneValue !== '' && addressValue !== ''){
    ...
    showAlert('You Have Update Success!!!', "success")
  } else{
    showAlert('You Have not Update Error!!!', "error")
  }
